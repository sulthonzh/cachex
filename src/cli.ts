...
const c = (t: string, col: string) => `${col}${t}${C.reset}`;
...
interface Opts { ttl?: number; maxSize?: number; format?: 'json'|'table'; file?: string; cacheName?: string; }

function parseArgs(args: string[]): { command: string; opts: Opts; rest: string[] } {
  const opts: Opts = {};
  const rest: string[] = [];
  let command = '';
  for (let i = 0; i < args.length; i++) {
    const a = args[i];
    if (a === '-t' || a === '--ttl') { opts.ttl = parseInt(args[++i]); }
    else if (a === '-m' || a === '--max-size' || a === '--maxSize') { opts.maxSize = parseInt(args[++i]); }
    else if (a === '-f' || a === '--format') { opts.format = args[++i] as 'json'|'table'; }
    else if (a === '-c' || a === '--cache') { opts.cacheName = args[++i]; }
    else if (a === '-o' || a === '--output' || a === '--file') { opts.file = args[++i]; }
    else if (a.startsWith('--')) { const [k,v] = a.slice(2).split('='); if (k==='ttl') opts.ttl=parseInt(v); else if (k==='maxSize'||k==='max-size') opts.maxSize=parseInt(v); else if (k==='format') opts.format=v as 'json'|'table'; else if (k==='cache') opts.cacheName=v; else if (k==='file'||k==='output') opts.file=v; }
...
function makeCache(opts: Opts): Cache | MultiCache {
  const co = { ttl: opts.ttl, maxSize: opts.maxSize };
  return opts.cacheName ? new MultiCache(co) : new Cache(co);
}

function isMulti(c: Cache | MultiCache): c is MultiCache { return c instanceof MultiCache; }

function saveToFile(cache: Cache | MultiCache, filename: string): void {
  let data: Record<string, any> = {};
  if (isMulti(cache)) {
    for (const name of cache.cacheNames()) {
      const s = cache.getStats(name); if (!s) continue;
      data[name] = { stats: s };
    }
  } else {
...
try {
  switch (command) {
    case 'set': {
      if (rest.length < 2) { console.error('Usage: cachex set <key> <value>'); process.exit(1); }
      let val: any; try { val = JSON.parse(rest[1]); } catch { val = rest[1]; }
      if (isMulti(cache)) cache.set(opts.cacheName!, rest[0], val, opts.ttl);
      else cache.set(rest[0], val, opts.ttl);
      console.log(c(`✓ Set ${rest[0]}`, C.green));
      if (opts.file) saveToFile(cache, opts.file);
      break;
    }
    case 'get': {
      if (rest.length < 1) { console.error('Usage: cachex get <key>'); process.exit(1); }
      const v = isMulti(cache) ? cache.get(opts.cacheName!, rest[0]) : cache.get(rest[0]);
      if (v === undefined) { console.log(c(`✗ ${rest[0]} not found`, C.red)); process.exit(1); }
      console.log(opts.format === 'json' ? JSON.stringify(v, null, 2) : typeof v === 'object' ? JSON.stringify(v, null, 2) : String(v));
      break;
    }
    case 'delete': {
      if (rest.length < 1) { console.error('Usage: cachex delete <key>'); process.exit(1); }
    ...
    case 'has': {
      if (rest.length < 1) { console.error('Usage: cachex has <key>'); process.exit(1); }
    ...
    case 'clear': {
      if (isMulti(cache)) cache.clear(opts.cacheName ?? ''); else cache.clear();
      console.log(c('✓ Cache cleared', C.green));
      break;
    }
    case 'stats': {
      const s = isMulti(cache) ? cache.getStats(opts.cacheName!) : cache.getStats();
      if (!s) { console.log(c('No stats available', C.yellow)); break; }
    ...
    case 'keys': {
      const ks = isMulti(cache) ? [] : (cache as Cache).keys();
      console.log(opts.format === 'json' ? JSON.stringify(ks) : ks.join(', '));
      break;
    }
    case 'values': {
      const vs = isMulti(cache) ? [] : (cache as Cache).values();
      console.log(opts.format === 'json' ? JSON.stringify(vs) : vs.map(v => JSON.stringify(v)).join('\n'));
      break;
    }
    case 'save': {
      if (rest.length < 1) { console.error('Usage: cachex save <file>'); process.exit(1); }
      saveToFile(cache, rest[0]);
      break;
    }
    case 'load': {
      if (rest.length < 1) { console.error('Usage: cachex load <file>'); process.exit(1); }
      if (!existsSync(rest[0])) { console.error(c(`✗ ${rest[0]} not found`, C.red)); process.exit(1); }
      const data = JSON.parse(readFileSync(rest[0], 'utf8'));
      if (!isMulti(cache)) {
        for (const [k, v] of Object.entries(data)) (cache as Cache).set(k, v);
      }
    ...
    case 'demo': {
      ...
      sc.get('user:1'); sc.get('user:1'); // generate hits
      sc.get('missing'); // generate miss
